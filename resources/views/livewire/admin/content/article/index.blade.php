            ...
            <div class="col">
                ...
                <ol class="breadcrumb breadcrumb-arrows" aria-label="breadcrumbs">
                    <li class="breadcrumb-item"><a
                            href="{{ route('admin.dashboard.index') }}">{{ __('bap.dashboard') }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page"><a
                            href="{{ route('admin.content.article.index') }}">{{ __('bap.articles') }}</a></li>
                </ol>
            </div>
            @can('admin_article_create')
                <div class="col-auto ms-auto d-print-none">
                    <div class="btn-list">
                        <button wire:click="$dispatch('showModal', {data: {'alias' : 'admin.content.article.create'}})"
                        ...
                        <button wire:click="$dispatch('showModal', {data: {'alias' : 'admin.content.article.create'}})"
                                class="btn btn-primary d-sm-none btn-icon" aria-label="{{ __('bap.create_article') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24"
                                 viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                 stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
    ...
    <div class="page-body">
                    ...
                    <div class="mx-2 d-inline-block">
                        <div class="btn-group btn-group-sm w-100">
                            <button type="button" wire:click="setPerPage(10)"
                                    class="btn @if($perPage == 10) btn-primary @endif">10
                            </button>
                            <button type="button" wire:click="setPerPage(15)"
                                    class="btn @if($perPage == 15) btn-primary @endif">15
                            </button>
                            <button type="button" wire:click="setPerPage(20)"
                                    class="btn @if($perPage == 20) btn-primary @endif">20
                            </button>
                            <button type="button" wire:click="setPerPage(25)"
                                    class="btn @if($perPage == 25) btn-primary @endif">25
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <div class="table-responsive">
            <table class="table card-table table-vcenter text-nowrap datatable">
                <thead>
                <tr>
                    <th class="w-1"><input name="selectAll" wire:model="selectAll"
                ...
                @foreach($articles as $article)
                    <tr>
                        <td><input class="form-check-input m-0 align-middle" type="checkbox" aria-label="Select User"
                                   value="{{ $article->id }}" name="selectedItems" wire:model="selectedItems"></td>
                        <td>{{ $article->id }}</td>
                        <td>
                        ...
                        <td class="text-end">
                            @can('admin_article_edit')
                                <button
                                    wire:click="$dispatch('showModal', { data:  {'alias' : 'admin.content.article.edit' , id: '{{ json_encode($article->id) }}' }})"
        ...
        <div class="card-footer d-flex justify-content-between">
            <div>
                <div class="btn-group btn-group-sm w-100">
                    <button type="button" wire:click="deleteSelected" class="btn">{{ __('bap.delete') }}
                        ({{ count($selectedItems) }})
                    </button>
                </div>
